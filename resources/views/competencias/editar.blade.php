    ...
    <div class="container">
        ...
        <form action="{{ route('competencias.update', $competencia->id) }}" method="post">
            ...
            <div class="form-group">
                ...
                <textarea id="descripcion" class="form-control{{ $errors->has('descripcion') ? ' is-invalid' : '' }}" name="descripcion" rows="3" cols="80" required>{{ $competencia->descripcion }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea id="resumen" class="form-control{{ $errors->has('resumen') ? ' is-invalid' : '' }}" name="resumen" rows="3" cols="80" required>{{ $competencia->resumen }}</textarea>
            ...
            <div class="form-group">
                ...
                <input id="codigo" type="number" class="form-control{{ $errors->has('codigo') ? ' is-invalid' : '' }}" name="codigo" value="{{ $competencia->codigo }}" required>
            ...
            <div class="form-group">
                ...
                <input id="duracionHoras" type="number" class="form-control{{ $errors->has('duracionHoras') ? ' is-invalid' : '' }}" name="duracionHoras" value="{{ $competencia->duracionHoras }}" required>
            ...
            <div class="form-group">
                ...
                @foreach ($programasFormacion as $key => $programaFormacion)
                    <div class="form-check">
                        <input type="checkbox" name="programa_formacion_id[]" id="programa_formacion_id-{{ $programaFormacion->id }}" value="{{ $programaFormacion->id }}" class="form-check-input" {{ $competencia->programasFormacion->contains($programaFormacion) ? 'checked' : '' }}>
                        <label for="programa_formacion_id-{{ $programaFormacion->id }}" class="form-check-label">
                            {{ __($programaFormacion->nombre.' ('.$programaFormacion->numeroFicha.')') }}
                        </label>
                    </div>
                @endforeach
