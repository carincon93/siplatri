...
@section('title', 'Crear resultado de aprendizaje')

@section('content')
    <div class="container">
        <a href="{{ url()->previous() }}" class="btn btn-sm btn-primary d-inline-block mb-4"><i class="fas fa-arrow-left"></i> Volver atrás</a>
        <form class="" action="{{ route('resultados_aprendizaje.store') }}" method="post">
            @csrf

            <div class="form-group">
                <label for="descripcion" class="col-form-label text-md-right">{{ __('Descripción') }} <span class="text-danger">*</span></label>
                <textarea id="descripcion" name="descripcion" rows="8" cols="80" class="form-control{{ $errors->has('descripcion') ? ' is-invalid' : '' }}" required>{{ old('descripcion') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select id="competencia_id" name="competencia_id" @change="obtenerHorasCompetencia" required>
                    ...
                    @forelse ($competencias as $key => $competencia)
                        <option value="{{ $competencia->id }}" {{ old('competencia_id') == $competencia->id ? 'selected' : '' }}>{{ $competencia->descripcion }}</option>
                    @empty
            ...
            <div class="form-group">
                ...
                <input id="horas" type="number" class="form-control{{ $errors->has('horas') ? ' is-invalid' : '' }}" name="horas" value="{{ old('horas') }}" min="0" required>
